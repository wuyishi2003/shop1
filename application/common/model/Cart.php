...
class Cart extends Common
{
    ...
    public function info($userId, $id = '', $display = '', $area_id = false, $point = 0, $coupon_code = "", $receipt_type = 1)
    {
        ...
        foreach($result['data']['list']as $k=>$v){
            ...
            if($v['nums']*$v['products']['dlprice'] == 0 ||$v['nums']*$v['products']['dlprice'] =='' ){
                $result['data']['list'][$k]['products']['amountdl'] = $v['nums']*$v['products']['price'];
            }else{
                $result['data']['list'][$k]['products']['amountdl'] = $v['nums']*$v['products']['dlprice'];
            }
            ...
            if($v['is_select']){
                ...
                //算订单总代理商品价格
                $result['data']['goods_amountdl'] += $result['data']['list'][$k]['products']['amountdl'];

                //算订单总价格
                $result['data']['amount'] += $result['data']['list'][$k]['products']['amount'];


                $result['data']['amountdl'] += $result['data']['list'][$k]['products']['amountdl'];
                //计算总重量
        ...
        if($receipt_type == 1)
        {
            if($area_id)
            {
                ...
                $result['data']['amountdl'] += $result['data']['cost_freight'];
            }
        }
        ...
        if($point != 0)
        {
            ...
            $max_point_deducted_moneydl = $result['data']['amountdl']*($orders_point_proportion/100); //代理最大积分抵扣的钱
            ...
            //$result['data']['amountdl'] = $result['data']['amountdl']-$point_deducted_moneydl;
        }

        $userModel = new User();
        $isdl = $userModel->isdl($userId);
        if($isdl==1  or empty($isdl)){
            unset( $result['data']['goods_amountdl'] );
            unset($result['data']['amountdl']);
        }else{
            $result['data']['amount'] = $result['data']['amountdl'];
        }

        $result['data']['wxsxf'] =$result['data']['amount'] *0.006;

...
class Goods extends Manage
{
    ...
    public function doAdd()
    {
        ...
        if ($open_spec) {
            ...
            $total_stock = $price = $costprice = $mktprice = $dlprice =0;
            $isExitDefalut = false;
            foreach ($product as $key => $val) {
                $tmp_product['goods']['price']        = isset($val['price']) ? $val['price'] : 0;


                $tmp_product['goods']['dlprice']     = isset($val['dlprice']) ? $val['dlprice'] : 0;
                ...
                if ($tmp_product['goods']['is_defalut'] == $productsModel::DEFALUT_YES) {//todo 取商品默认价格
                    ...
                    $dlprice     = $tmp_product['goods']['dlprice'];
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        $data['goods']['dlprice']         = input('post.goods.dlprice', '');

        $data['goods']['p']         = input('post.goods.p', '');
        $data['goods']['pp']         = input('post.goods.pp', '');
    ...
    private function checkProductInfo($data, $goods_id = 0, $isEdit = false)
    {
        ...
        $data['product']['dlprice']      = $data['goods']['dlprice'];//代理价格
    ...
    public function getSpecHtml()
    {
        ...
        if ($spec) {
            ...
            foreach ((array)$items as $key => $val) {
                ...
                $items[$key]['dlprice']     = $goodsDefault['dlprice'];
    ...
    public function doEdit()
    {
        ...
        if ($open_spec) {
            ...
            foreach ($product as $key => $val) {
                ...
                $tmp_product['goods']['dlprice']    = !empty($val['dlprice']) ? $val['dlprice'] : 0;
                $tmp_product['goods']['costprice']    = !empty($val['costprice']) ? $val['costprice'] : 0;

                $tmp_product['goods']['p']  = input('post.goods.p', '');
                $tmp_product['goods']['pp']         = input('post.goods.pp', '');
